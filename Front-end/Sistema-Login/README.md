# 🟡 Sistema Login

O objetivo do projeto é desenvolver um sistema de verificação de login. O sistema valida a senha informada pelo usuário e permite o acesso apenas quando a senha estiver correta. Caso a senha seja inválida, o acesso é negado.

![Prévia do Projeto](../Img/Sistema-Login.png)
![Prévia do Projeto](../Img/Sistema-Login2.png)


---
## 🟡 Funcionalidades

- Verificador de Produtos com descontos
- Validação de campos
  
---
## 🟡 Tecnologias Utilizadas

- HTML5
- JavaScript (ES6+)
- VS Code
- Git e GitHub

---
## 🟡 Como rodar o projeto​?

1 Baixe o repositório (Clique no Botão “Code” > Download ZIP)​

2 Extraia a pasta​

3 Abra o arquivo index.html no navegador​

## Acesse o projeto online
  https://luiscamara123.github.io/Senac/Front-end/Sistema-Login/

---
## 🟡 Como clonar o projeto​

**No terminal do VS Code digite:​**
  git clone https://github.com/Luiscamara123/Senac.git

No GitHub Desktop:​
  No menu: File(Arquivo) > Clone a repositor(Clonar repositório) > Selecione a aba URL, no campo URL, cole o link do repositório 

https://github.com/Luiscamara123/Senac.git

---
## 🟡 Melhorias futuras

- Organização na estrutura de códigos
- Aplicação do Arquivo de estilização CSS
- Funções com a Linguagem JavaScrpit
- Banco de Dados

---
## 🟡 O que eu aprendi?

- Estrutura e validação de códigos com JavaScript

---
## 🟡 Autor

**Luís Felipe Câmara Alcântara**

Turma de Tecnologia em Informática para internet (Vespertino) - Senac DF




